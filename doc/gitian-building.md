...
*Setup instructions for a Gitian build of ExpoCash Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the ExpoCash
...
- [Building ExpoCash Core](#building-expocash-core)
...
Clone the git repositories for expocash and Gitian.
...
in the expocash repository under 'Fetch and create inputs' to install sources which require
manual intervention. Also optionally follow the next step: 'Seed the Gitian sources cache
and offline git repositories' which will fetch the remaining files required for building
offline.

Building ExpoCash Core
----------------

To build ExpoCash Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the expocash repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/expocash/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/expocash/expocash
...
For example:
```bash
URL=https://github.com/thrasher-/expocash.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit expocash=${COMMIT} --url expocash=${URL} ../expocash/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit expocash=${COMMIT} --url expocash=${URL} ../expocash/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit expocash=${COMMIT} --url expocash=${URL} ../expocash/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the expocash git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=trusty on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../expocash/contrib/gitian-descriptors/*|sort|uniq )
...
BTCPATH=/some/root/path/expocash
SIGPATH=/some/root/path/expocash-detached-sigs

./bin/gbuild --url expocash=${BTCPATH},signature=${SIGPATH} ../expocash/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/expocash-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/expocash-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/expocash-osx-build.assert
```
...
[expocash-project/gitian.sigs.expo](https://github.com/expocash-project/gitian.sigs.expo/) repository, or if that's not possible create a pull
