...
Below are some notes on how to build ExpoCash Core for Windows.

The options known to work for building ExpoCash Core on Windows are:

* On Linux using the [Mingw-w64](https://mingw-w64.org/doku.php) cross compiler tool chain. Ubuntu Trusty 14.04 is recommended
and is the platform used to build the ExpoCash Core Windows release binaries.
...
for Ubuntu Xenial does not produce working executables for some of the ExpoCash Core applications.
...
Note that for WSL the ExpoCash Core source path MUST be somewhere in the default mount file system, for
example /usr/src/expocash, AND not under /mnt/d/. If this is not the case the dependency autoconf scripts will fail.
This means you cannot use a directory that located directly on the host Windows file system to perform the build.

The next three steps are an example of how to acquire the source in an appropriate way.

    cd /usr/src
    sudo git clone https://github.com/expocash/expocash.git
    sudo chmod -R a+rw expocash
...
way. This will install to `c:\workspace\expocash`, for example:

    make install DESTDIR=/mnt/c/workspace/expocash
...
causes two of the expocash executables to crash shortly after start up. The bug is related to the
...
It's not possible to build the expocash code using the win32 version of the Mingw-w64 cross compilers (at least not without
modifying headers in the expocash source code).
