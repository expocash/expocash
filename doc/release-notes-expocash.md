ExpoCash Core version *0.15.1* is now available from:

  <https://download.expo.cash/expocash-0.15.1/>
...
Please report bugs using the issue tracker at GitHub:
  ...
  <https://github.com/expocash/expocash/issues>

To receive security and update notifications, please subscribe to:

  <https://groups.google.com/forum/#!forum/expocash-dev>
...
installer (on Windows) or just copy over `/Applications/ExpoCash-Qt` (on Mac)
or `expocashd`/`expocash-qt` (on Linux).
...
ExpoCash Core is extensively tested on multiple operating systems using
the Linux kernel, macOS 10.8+, and Windows Vista and later. Windows XP is not supported.

ExpoCash Core should also work on most other Unix-like systems but is not
...
A number of changes to the way ExpoCash Core deals with peer connections and invalid blocks
...
The minimum relay fee `-minrelayfee` has been lowered to 0.01 lites per kB (0.00001 EXPO). This is to prepare for dropping the minimum transaction fee to 0.1 lites per kB (0.0001 EXPO) in the next release.
