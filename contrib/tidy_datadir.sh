...
if [ -d "$1" ]; then
  ...
  echo "Removes obsolete ExpoCash database files" >&2
  exit 1
fi
...
case $LEVEL in
  0)
    echo "Error: no ExpoCash datadir detected."
    exit 1
    ;;
  1)
    echo "Detected old ExpoCash datadir (before 0.7)."
    echo "Nothing to do."
    exit 0
    ;;
  2)
    echo "Detected ExpoCash 0.7 datadir."
    ;;
  3)
    echo "Detected ExpoCash pre-0.8 datadir."
    ;;
  4)
    echo "Detected ExpoCash 0.8 datadir."
    ;;
esac
